...
stepFailed() {
  if (("$#" < 2)); then
    echo "Usage: $0 <Step name> <Error output>"
    exit 1
  fi
...
fetchRepository() {
  ...
  if [[ -d "${worksDir}/${workName}" ]]; then
    cd "${worksDir}/${workName}" &&
      git clean -fd && git fetch origin && git reset --hard origin/solution
    return $?
  fi
...
pullSolutionBranch() {
  message=$(git fetch origin && git reset --hard origin/solution)
  if [ "$?" != "0" ]; then
    echo "$message"
    return 1
  fi
...
checkMakeBuild() {
  ...
  message=$(SOME_BUILD_FLAGS="-Wextra -Werror -Wall -Wfloat-equal -pedantic" make 2>/dev/stdout)
  if [ "$?" != "0" ]; then
    echo "$message"
    return 1
  fi
