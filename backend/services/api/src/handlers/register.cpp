...
RegisterHandler::HandleRequestJsonThrow(const HttpRequest &request,
  ...
  auto res = transaction.Execute(sql::kIsStudentExists, body.gv_name);
  ...
  res = transaction.Execute(sql::kInsertStudent, body.gv_name, body.group,
                            body.first_name, body.last_name, body.father_name,
                            initials, body.email, passwordHash);
  if (res.RowsAffected()) {
    transaction.Commit();
    request.SetResponseStatus(server::http::HttpStatus::kCreated);
    return formats::json::ValueBuilder{
        registration::RegisterResponseBody{"success", "Registered"}}
        .ExtractValue();
  }
